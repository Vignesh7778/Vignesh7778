<div align="center">

<!-- Local Animated Hero Header SVG -->
<img src="assets/hero.svg" alt="Vicky Terminal Hero" width="100%" />

<br/>

```text
┌────────────────────────────────────────────────────────────────────────────────────────┐
│ [0.000000] Linux kernel 6.8.0-vignesh-custom-x86_64 #1 SMP PREEMPT_DYNAMIC             │
│ [0.042100] Initializing Developer Environment...                           [ OK ]      │
│ [0.108420] Mounting File System /dev/nvme0n1p2 (Ext4)...                   [ OK ]      │
│ [0.254190] Establishing Secure Handshake with github.com/Vignesh7778...    [ OK ]      │
│ [0.412080] Loading Security Modules & Neural AI Pipelines...               [ OK ]      │
│ [0.589120] Welcome to Ubuntu 24.04.1 LTS (GNU/Linux 6.8.0-custom-vignesh)              │
│ [0.720150] User Session Authenticated: vignesh@ubuntu-terminal                         │
└────────────────────────────────────────────────────────────────────────────────────────┘
```

<br/>

```text
┌────────────────────────────────────────────────────────────────────────────────────────┐
│ vignesh@ubuntu:~$ whoami                                                               │
└────────────────────────────────────────────────────────────────────────────────────────┘
```

# 👋 Hi, I'm Vicky <img src="https://raw.githubusercontent.com/MartinLaxenaire/MartinLaxenaire/main/src/waving-hand.gif" width="35px" alt="Waving Hand" />

<a href="https://github.com/Vignesh7778">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&size=24&duration=3000&pause=1000&color=00FF66&center=true&vCenter=true&width=850&lines=%F0%9F%92%BB+Full+Stack+Developer;%F0%9F%94%92+Cybersecurity+Enthusiast;%F0%9F%A9%B9+Problem+Solver;%E2%98%95+Passionate+about+Java" alt="Animated Typing Banner" />
</a>

<br/><br/>

<!-- Real-time Status Badges -->
<a href="https://github.com/Vignesh7778">
  <img src="https://komarev.com/ghpvc/?username=Vignesh7778&color=00ff66&style=for-the-badge&label=TERMINAL+VISITORS" alt="Profile Views" />
</a>
<a href="https://github.com/Vignesh7778?tab=followers">
  <img src="https://img.shields.io/github/followers/Vignesh7778?label=FOLLOWERS&style=for-the-badge&color=00D2FF&logo=github&logoColor=white" alt="GitHub Followers" />
</a>
<a href="https://github.com/Vignesh7778">
  <img src="https://img.shields.io/badge/KERNEL%20SECURITY-SYSTEM%20HARDENED-00FF66?style=for-the-badge&logo=shield&logoColor=black" alt="Security Status" />
</a>
<a href="https://github.com/Vignesh7778">
  <img src="https://img.shields.io/badge/SESSION%20STATUS-OPEN%20FOR%20HIRE-00D2FF?style=for-the-badge&logo=briefcase&logoColor=white" alt="Hire Status" />
</a>

<br/><br/>

<!-- Quick CTA Buttons -->
<a href="#-execute-cat-aboutmd">
  <img src="https://img.shields.io/badge/%F0%9F%9A%80_RUN_./EXPLORE.SH-00FF66?style=for-the-badge&logoColor=black" alt="Explore CTA" />
</a>
<a href="https://new-portfolio-gold-alpha.vercel.app">
  <img src="https://img.shields.io/badge/%F0%9F%8C%90_LAUNCH_LIVE_PORTFOLIO-161B22?style=for-the-badge&logo=vercel&logoColor=00FF66" alt="Live Portfolio" />
</a>
<a href="https://www.linkedin.com/in/vignesh-murali-dharan/">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
</a>
<a href="mailto:vigneshdevi22@gmail.com">
  <img src="https://img.shields.io/badge/Email_Me-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
</a>

</div>

<br/>

<div align="center">
  <img src="assets/divider.svg" width="100%" />
  <br/>
  <sub>↓ <b>SCROLL TO EXECUTE TERMINAL COMMAND PIPELINE</b> ↓</sub>
</div>

<br/>

---

## 🖥️ Command 01: `$ uname -a` & `$ cat /etc/os-release`

```bash
vignesh@ubuntu:~$ uname -a && cat /etc/os-release
```

<div align="center">

<img src="assets/portrait.svg" alt="Animated ASCII Portrait Terminal" width="48%" />
&nbsp;
<img src="assets/wordmark.svg" alt="3D Animated Wordmark Terminal" width="48%" />

<br/><br/>

<img src="assets/terminal.svg" alt="Terminal System Diagnostics" width="98%" />

<br/><br/>

| System Property | Developer Diagnostics & Specifications |
| :--- | :--- |
| 👤 **User Identifier** | **Vicky** (`vignesh@ubuntu-terminal`) |
| 🎓 **Academic Status** | **B.E. Computer Science & Engineering (Specialization: Cyber Security)** — Final Year Student |
| 💻 **Core Identity** | **Full Stack Developer** & **Software Engineer** |
| ☕ **Primary Language** | **Java (JDK 17 / JDK 21)** & **Spring Boot Microservices** |
| 🎨 **Frontend Architecture** | **Angular, React, JavaScript (ES6+), HTML5, CSS3, TailwindCSS** |
| 🛡️ **Cyber Security Vector** | **Application Hardening, Cryptography, OWASP Auditing, Network Defense** |
| 🤖 **Artificial Intelligence** | **Autonomous AI Agents, Prompt Engineering, LLM Pipeline Integrations** |
| 🖥️ **Operating Environment** | **Ubuntu 24.04.1 LTS x86_64 (Linux Kernel 6.8.0)** |

</div>

<br/>

<div align="center">
  <img src="assets/divider.svg" width="100%" />
</div>

<br/>

---

## 👤 <a id="-execute-cat-aboutmd"></a>Command 02: `$ cat about.md`

```bash
vignesh@ubuntu:~$ cat about.md
```

> **Final Year Computer Science & Engineering Student** specializing in **Cyber Security**. I engineer resilient, high-throughput backend services paired with intuitive frontend architectures and hardened security layers as a **Full Stack Developer**.

<br/>

<details open>
<summary><b>🔍 Click to Expand System Bio & Engineering Principles</b></summary>

<br/>

```text
+-----------------------------------------------------------------------------------------+
| TERMINAL DIAGNOSTICS & SYSTEM SUMMARY                                                  |
+-----------------------------------------------------------------------------------------+
| [✓] BACKEND       : Java, Spring Boot, RESTful APIs, Hibernate/JPA, Microservices      |
| [✓] FRONTEND      : Angular, React, Responsive UI, State Management (RxJS/Redux)        |
| [✓] SECURITY      : Zero Trust Architecture, OWASP Top 10 Mitigation, Cryptography      |
| [✓] AI WORKFLOWS  : Autonomous AI Agents, Prompt Engineering, LLM Integrations          |
| [✓] DATABASE      : Relational Schema Optimization, MySQL, Firebase BaaS                |
+-----------------------------------------------------------------------------------------+
```

- 🎓 **Education:** Final Year B.E. Computer Science Engineering (*Specialization: Cyber Security*)
- 💻 **Core Role:** Full Stack Developer
- ☕ **Backend Stack:** Java Ecosystem, Spring Boot Microservices, Data Structures & Algorithms
- 🤖 **AI & Automation:** Building intelligent AI Agents, autonomous task workflows, and LLM integrations
- 🎨 **Frontend Craftsmanship:** Creating sleek, user-centric web applications using Angular & React
- 🛡️ **Cyber Security Focus:** Vulnerability assessments, secure coding protocols, and system defense
- 🌐 **Open Source & Growth:** Active contributor to developer communities, open-source projects & hackathons

</details>

<br/>

<div align="center">
  <img src="assets/divider.svg" width="100%" />
</div>

<br/>

---

## 🛠️ Command 03: `$ ls -la --group-directories-first skills/`

```bash
vignesh@ubuntu:~$ ls -la --group-directories-first skills/
```

<div align="center">

### ☕ `bin/languages/` (Languages & Core Runtimes)
<a href="https://www.java.com/">
  <img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white" alt="Java" />
</a>
<a href="https://spring.io/projects/spring-boot">
  <img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white" alt="Spring Boot" />
</a>
<a href="https://www.python.org/">
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" alt="Python" />
</a>
<a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript">
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black" alt="JavaScript" />
</a>
<a href="https://www.mysql.com/">
  <img src="https://img.shields.io/badge/SQL_&_Relational-003B57?style=for-the-badge&logo=postgresql&logoColor=white" alt="SQL" />
</a>

<br/>

### 🎨 `lib/frontend/` (UI Frameworks & Styling)
<a href="https://angular.io/">
  <img src="https://img.shields.io/badge/Angular-DD0031?style=for-the-badge&logo=angular&logoColor=white" alt="Angular" />
</a>
<a href="https://reactjs.org/">
  <img src="https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=black" alt="React" />
</a>
<a href="https://developer.mozilla.org/en-US/docs/Web/HTML5">
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white" alt="HTML5" />
</a>
<a href="https://developer.mozilla.org/en-US/docs/Web/CSS">
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white" alt="CSS3" />
</a>

<br/>

### 🗄️ `var/db/` (Databases & Cloud Storage)
<a href="https://www.mysql.com/">
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white" alt="MySQL" />
</a>
<a href="https://firebase.google.com/">
  <img src="https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black" alt="Firebase" />
</a>

<br/>

### 🧰 `etc/tools/` (DevOps & Security Utilities)
<a href="https://git-scm.com/">
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" alt="Git" />
</a>
<a href="https://github.com/">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
</a>
<a href="https://code.visualstudio.com/">
  <img src="https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white" alt="VS Code" />
</a>
<a href="https://www.postman.com/">
  <img src="https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white" alt="Postman" />
</a>
<a href="https://www.docker.com/">
  <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white" alt="Docker" />
</a>

</div>

<br/>

<div align="center">
  <img src="assets/divider.svg" width="100%" />
</div>

<br/>

---

## 🚀 Command 04: `$ ./projects.sh --list-featured`

```bash
vignesh@ubuntu:~$ ./projects.sh --list-featured
```

<div align="center">

| Project Module | Technology Stack | Status | Actions / Demo |
| :--- | :--- | :---: | :---: |
| 🚜 **CropLedger Desk**<br/>*Blockchain-inspired Agricultural Ledger* | `Java` `Spring Boot`<br/>`MySQL` `React` | ![Production](https://img.shields.io/badge/STATUS-PRODUCTION-00FF66?style=flat-square) | [<img src="https://img.shields.io/badge/Code-181717?style=flat-square&logo=github&logoColor=white" />](https://github.com/Vignesh7778) [<img src="https://img.shields.io/badge/Demo-000000?style=flat-square&logo=vercel&logoColor=00FF66" />](https://new-portfolio-gold-alpha.vercel.app) |
| 📚 **EduAura**<br/>*Interactive Learning & Analytics Platform* | `Angular` `Spring Boot`<br/>`Firebase` `MySQL` | ![Production](https://img.shields.io/badge/STATUS-PRODUCTION-00FF66?style=flat-square) | [<img src="https://img.shields.io/badge/Code-181717?style=flat-square&logo=github&logoColor=white" />](https://github.com/Vignesh7778) [<img src="https://img.shields.io/badge/Demo-000000?style=flat-square&logo=vercel&logoColor=00FF66" />](https://new-portfolio-gold-alpha.vercel.app) |
| 🤖 **AI Agent**<br/>*Autonomous Task Workflow Assistant* | `Python` `Java`<br/>`LLM APIs` `Docker` | ![Active](https://img.shields.io/badge/STATUS-ACTIVE-00D2FF?style=flat-square) | [<img src="https://img.shields.io/badge/Code-181717?style=flat-square&logo=github&logoColor=white" />](https://github.com/Vignesh7778) [<img src="https://img.shields.io/badge/Demo-000000?style=flat-square&logo=vercel&logoColor=00FF66" />](https://new-portfolio-gold-alpha.vercel.app) |
| 🌐 **Portfolio Website**<br/>*Modern High-Performance Developer Hub* | `React` `JavaScript`<br/>`CSS3` `Vercel` | ![Maintained](https://img.shields.io/badge/STATUS-STABLE-00FF66?style=flat-square) | [<img src="https://img.shields.io/badge/Code-181717?style=flat-square&logo=github&logoColor=white" />](https://github.com/Vignesh7778/Vignesh7778) [<img src="https://img.shields.io/badge/Demo-000000?style=flat-square&logo=vercel&logoColor=00FF66" />](https://new-portfolio-gold-alpha.vercel.app) |
| 🔐 **Cyber Security Toolkit**<br/>*Vulnerability Scanner & Hash Auditor* | `Python` `Java`<br/>`Network Security` | ![Hardened](https://img.shields.io/badge/STATUS-SECURE-00D2FF?style=flat-square) | [<img src="https://img.shields.io/badge/Code-181717?style=flat-square&logo=github&logoColor=white" />](https://github.com/Vignesh7778) [<img src="https://img.shields.io/badge/Demo-000000?style=flat-square&logo=vercel&logoColor=00FF66" />](https://new-portfolio-gold-alpha.vercel.app) |

</div>

<br/>

<details>
<summary><b>📖 Click to Expand Deep Technical Architecture & Project Details</b></summary>

<br/>

### 🚜 1. CropLedger Desk
- **Problem Statement:** Lack of transparency and transaction security in agricultural supply chain tracking.
- **Solution:** Engineered a Java & Spring Boot backend with immutable transaction logging, MySQL relational storage, and a dynamic React desktop dashboard.
- **Security Protocols:** Enforced SHA-256 data hashing for ledger entries and JWT authentication for users.

### 📚 2. EduAura
- **Problem Statement:** Inefficient student performance tracking and fragmented learning resources.
- **Solution:** Developed a feature-rich learning portal using Angular for component modularity and Spring Boot for RESTful API orchestration.
- **Features:** Real-time analytics, dynamic quiz evaluation, and role-based access control (RBAC).

### 🤖 3. AI Agent
- **Problem Statement:** Manual execution of repetitive multi-step API workflows.
- **Solution:** Built an autonomous AI workflow assistant leveraging Python, custom prompt orchestration, and LLM APIs to handle autonomous task execution.
- **Deployment:** Containerized using Docker for reproducible execution environments.

### 🌐 4. Portfolio Website
- **Problem Statement:** Standard static portfolios lack developer personality and terminal responsiveness.
- **Solution:** Designed a ultra-responsive portfolio featuring dark terminal themes, micro-animations, fast asset rendering, and Vercel edge deployment.

### 🔐 5. Cyber Security Toolkit
- **Problem Statement:** Need for a lightweight diagnostic suite for basic security auditing and vulnerability identification.
- **Solution:** Authored a Python & Java security suite featuring multi-threaded port scanners, payload generator modules, and hash cracking verification utilities.

</details>

<br/>

<div align="center">
  <img src="assets/divider.svg" width="100%" />
</div>

<br/>

---

## 📊 Command 05: `$ ./show_stats.sh --all`

```bash
vignesh@ubuntu:~$ ./show_stats.sh --user=Vignesh7778
```

<div align="center">

  <!-- Automated Contribution Snake SVG -->
  <img src="https://raw.githubusercontent.com/Vignesh7778/Vignesh7778/output/github-contribution-grid-snake-dark.svg" alt="GitHub Contribution Snake" width="95%" />

  <br/><br/>

  <!-- GitHub Trophies Widget -->
  <img src="https://github-profile-trophy.vercel.app/?username=Vignesh7778&theme=tokyonight&no-bg=true&margin-w=15&row=1" alt="GitHub Profile Trophies" />

  <br/><br/>

  <!-- Stats & Top Languages Side by Side -->
  <img src="https://github-readme-stats.vercel.app/api?username=Vignesh7778&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=00ff66&icon_color=00d2ff&text_color=c9d1d9" alt="Vignesh's GitHub Stats" height="175" />
  &nbsp;
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Vignesh7778&layout=compact&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=00ff66&text_color=c9d1d9" alt="Top Languages" height="175" />

  <br/><br/>

  <!-- GitHub Streak Widget -->
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Vignesh7778&theme=tokyonight&hide_border=true&background=0D1117&ring=00FF66&fire=00FF66&currStreakLabel=00D2FF" alt="GitHub Streak" height="175" />

  <br/><br/>

  <!-- GitHub Activity Graph -->
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=Vignesh7778&theme=tokyonight&bg_color=0d1117&color=00ff66&line=00d2ff&point=ffffff&hide_border=true" alt="Activity Graph" width="95%" />

</div>

<br/>

<div align="center">
  <img src="assets/divider.svg" width="100%" />
</div>

<br/>

---

## 💼 Command 06: `$ history | grep -E "(education|internship|hackathon|milestones)"`

```bash
vignesh@ubuntu:~$ history | grep -E "(education|internship|hackathon|milestones)"
```

<div align="center">

| Milestone / Timeline | Role & Institution | Key Contributions & Achievements |
| :---: | :--- | :--- |
| 🎓 **2021 — 2025** | **B.E. Computer Science Engineering (Cyber Security)**<br/>*Final Year Undergraduate* | Core coursework in Data Structures, Algorithms, Operating Systems, Database Management Systems, Network Defense, and Applied Cryptography. |
| 💼 **2023 — 2024** | **Full Stack Software Development Intern**<br/>*Tech Solutions & Software Firms* | Designed and optimized Spring Boot RESTful services, implemented database schema migrations, and collaborated on dynamic Angular/React UI modules. |
| 🏆 **2023 — 2025** | **Hackathons & Coding Competitions**<br/>*Participant & Winner* | Built functional full-stack prototypes under tight deadlines, featuring secure blockchain-inspired supply chain tracking and AI workflow agents. |
| 📜 **2024 — Present** | **Professional Certifications**<br/>*Google AI & Cyber Security* | Earned specialized certifications in Google AI Essentials, Google Cloud Skills, and Cyber Security Practices. |

</div>

<br/>

<div align="center">
  <img src="assets/divider.svg" width="100%" />
</div>

<br/>

---

## ⚡ Command 07: `$ top -b -n 1 -u vignesh | grep -i LEARNING`

```bash
vignesh@ubuntu:~$ top -b -n 1 -u vignesh | grep -i LEARNING
```

```text
PID   USER      TASK                         PROGRESS                                STATUS
1001  vignesh   Spring Boot Microservices    [█████████████████████████████████░░░]  88% ADVANCED
1002  vignesh   React & Frontend Patterns    [█████████████████████████████████░░░]  85% ADVANCED
1003  vignesh   Docker & Kubernetes Containers [██████████████████████████████░░░░░░]  80% IN_PROGRESS
1004  vignesh   System Design & Architecture  [██████████████████████████████░░░░░░]  78% IN_PROGRESS
1005  vignesh   LLMs & AI Workflow Agents    [█████████████████████████████████░░░]  85% ACTIVE
1006  vignesh   Microservices Security       [██████████████████████████████████░░]  86% ACTIVE
1007  vignesh   Advanced DSA & Problem Solving[████████████████████████████████████]  92% EXPERT
```

<br/>

<details>
<summary><b>🚀 Click to Expand Detailed Learning Goals & Targets</b></summary>

<br/>

- 🎯 **Target 1:** Architecting distributed event-driven microservices using Spring Cloud & Kafka.
- 🎯 **Target 2:** Containerizing full-stack microservice apps with Docker & orchestrating deployments via Kubernetes.
- 🎯 **Target 3:** Mastering high-throughput system design patterns (Caching with Redis, Load Balancing, Rate Limiting).
- 🎯 **Target 4:** Building context-aware RAG (Retrieval-Augmented Generation) applications and autonomous AI agents.

</details>

<br/>

<div align="center">
  <img src="assets/divider.svg" width="100%" />
</div>

<br/>

---

## 📜 Command 08: `$ ls -l certificates/`

```bash
vignesh@ubuntu:~$ ls -l certificates/
```

<div align="center">

<a href="https://github.com/Vignesh7778">
  <img src="https://img.shields.io/badge/Google-AI_Essentials-4285F4?style=for-the-badge&logo=google&logoColor=white" alt="Google AI" />
</a>
<a href="https://github.com/Vignesh7778">
  <img src="https://img.shields.io/badge/Google_Cloud-Skills_Boost-4285F4?style=for-the-badge&logo=googlecloud&logoColor=white" alt="Google Cloud" />
</a>
<a href="https://github.com/Vignesh7778">
  <img src="https://img.shields.io/badge/Cyber_Security-Specialization-00D2FF?style=for-the-badge&logo=shield&logoColor=white" alt="Cyber Security" />
</a>
<a href="https://github.com/Vignesh7778">
  <img src="https://img.shields.io/badge/Software_Internship-Completed-00FF66?style=for-the-badge&logo=checkmarx&logoColor=black" alt="Internship" />
</a>
<a href="https://github.com/Vignesh7778">
  <img src="https://img.shields.io/badge/Hackathon-Innovator_Badge-FFD700?style=for-the-badge&logo=trophy&logoColor=black" alt="Hackathon" />
</a>

</div>

<br/>

<details>
<summary><b>📜 Click to Expand Certificate Verifications & Skills Covered</b></summary>

<br/>

- 🤖 **Google AI Essentials:** Prompt Engineering, Generative AI principles, and workflow integration.
- ☁️ **Google Cloud Certification:** Cloud infrastructure fundamentals, IAM, compute engine, and security tools.
- 🛡️ **Cyber Security Specialization:** Network security, ethical hacking techniques, OWASP mitigation, and system auditing.
- 💼 **Full Stack Developer Internship:** Real-world software development lifecycle, RESTful services, and database tuning.
- 🏆 **Hackathon Certificates:** Project awards for rapid prototyping and innovative solution engineering.

</details>

<br/>

<div align="center">
  <img src="assets/divider.svg" width="100%" />
</div>

<br/>

---

## 🌐 Command 09: `$ connect --all-networks`

```bash
vignesh@ubuntu:~$ connect --all-networks
```

<div align="center">

<a href="https://github.com/Vignesh7778">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
</a>
<a href="https://www.linkedin.com/in/vignesh-murali-dharan/">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
</a>
<a href="https://leetcode.com/u/M_Vignesh_1520/">
  <img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black" alt="LeetCode" />
</a>
<a href="https://www.hackerrank.com/profile/Vignesh7778">
  <img src="https://img.shields.io/badge/HackerRank-2EC866?style=for-the-badge&logo=hackerrank&logoColor=white" alt="HackerRank" />
</a>
<a href="https://www.geeksforgeeks.org/user/vignesh7778/">
  <img src="https://img.shields.io/badge/GeeksforGeeks-298D46?style=for-the-badge&logo=geeksforgeeks&logoColor=white" alt="GeeksforGeeks" />
</a>
<a href="https://www.codechef.com/users/vignesh7778">
  <img src="https://img.shields.io/badge/CodeChef-5B4638?style=for-the-badge&logo=codechef&logoColor=white" alt="CodeChef" />
</a>
<a href="https://new-portfolio-gold-alpha.vercel.app">
  <img src="https://img.shields.io/badge/Live_Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=00FF66" alt="Portfolio" />
</a>
<a href="mailto:vigneshdevi22@gmail.com">
  <img src="https://img.shields.io/badge/Email_Me-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
</a>

</div>

<br/>

<div align="center">
  <img src="assets/divider.svg" width="100%" />
</div>

<br/>

---

## 💡 Command 10: `$ sudo hire me` & `$ exit`

```bash
vignesh@ubuntu:~$ echo $FUN_FACT
```

```text
"I love turning complex ideas into clean, resilient, real-world software."
```

<br/>

```bash
vignesh@ubuntu:~$ sudo hire me
```

```text
[sudo] password for recruiter: *********
Access Granted. Initializing recruitment pipeline...
Status: Ready for technical interviews & high-impact engineering roles!
```

<br/>

```bash
vignesh@ubuntu:~$ exit
logout
Saving session...
...copying shared history...
...saving history...truncating history files...
...completed.
Closing connection to 127.0.0.1.
```

<br/>

---

<div align="center">

<img src="assets/footer.svg" width="100%" alt="Footer Banner" />

<br/>

**Happy Coding 🚀**

<sub>Engineered with ❤️ and Futuristic Terminal Aesthetics by [Vicky](https://github.com/Vignesh7778)</sub>

</div>
